...
def rank_vis(query_mat, gallery_mat, n_match, save_dir):
    ...
    for i in tqdm(range(num_query)):
        ...
        junk_flag = np.equal(gallery_ids, -1)
        ...
        plt.figure(figsize=(4 * n_match, 30))
        ...
        if not os.path.basename(q_img_path) == '0115_c1_f0073267.jpg':
            continue

        # --------------------------------- #
        plt.subplot(3, n_match + 1, 1)
...
if __name__ == '__main__':
    ...
    parser.add_argument("dataset", type=str, choices=["market", "duke", "msmt", "cuhk"])
    parser.add_argument("prefix", type=str)
    args = parser.parse_args()

    q_mat_path = os.path.join("features", args.dataset, "query-%s.mat" % args.prefix)
    g_mat_path = os.path.join("features", args.dataset, "gallery-%s.mat" % args.prefix)

    g_mat = sio.loadmat(g_mat_path)
    q_mat = sio.loadmat(q_mat_path)

    rank_vis(q_mat, g_mat, n_match=40, save_dir="/home/chuanchen_luo")
